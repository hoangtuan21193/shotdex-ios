...
- **Standard-iOS UI (tier A/B) gets a HIG pass**: the `hig-components` agent (`.claude/agents/hig-components.md`) reviews a screen against Apple's Human Interface Guidelines *Components* pages, fetching the page rather than answering from memory. Run it when adding or reworking a toolbar, menu, sheet, alert, picker or list, and read `DESIGN.md` first — tier D (editor, collage, video studio, compare) is deliberately non-standard, so only target sizes, clipped text, accessibility labels, unconfirmed destructive actions and lying control state count there.
