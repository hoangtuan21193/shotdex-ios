...
- **ShotDex is unreleased — no backward compatibility for saved edits** (decided 2026-09-23). Changing the meaning of a `PhotoEditRecipe` / `PhotoAdjustments` key, reordering the render pipeline, or renaming an enum's raw value needs **no** migration of old values, **no** version flag, and **no** "old photos will render differently" release note — and do not ask the user about it. Two things still hold because they are robustness, not compatibility: decode stays lossy (`LossyArray` in `ShotDexKit/Models/LossyDecoding.swift` — one unreadable element must not take the whole recipe or all of My Looks with it), and user-typed data stays out of `photo_metadata` (the indexer rewrites those rows in the running app, released or not). Revisit this rule the day the app ships.
...
- **Hooks are the approval gates** (`.claude/settings.json` + `.claude/hooks/`): pushing past the gate is blocked, archive/upload needs `RELEASE_APPROVAL`, and `SHOTDEX_FREEZE_TESTS=1` freezes test files while fixing a bug (write the failing test first). A Stop hook (`build-check.py`, fed by `mark-dirty.py`) rebuilds after any session that touched source and blocks stopping while the Xcode Issue navigator shows **any error or warning, project-wide** — it reads the per-file `.dia` diagnostics so warnings from files the incremental build skipped still count. Fix them, or tell the user why one should stay; it lets go after 3 rounds. `SHOTDEX_BUILD_HOOK=0` turns it off.
...
- **Review agents live in `.claude/agents/`** — run the one that matches the work and act on its findings rather than re-deriving them:
  ...
  - **Platform feel, one agent per screen size** — how it feels in the hand, not what it looks like: gestures and what they fight with, push-vs-sheet and where Back lands, motion, haptics, perceived latency, hierarchy, first run. All three read `.claude/reference/ios-ux-doctrine.md` for the shared half and cite the HIG **Foundations/Patterns** pages, where `hig-components` cites Components. `device-layout` measures sizes; these judge the feel at that size.
    - `iphone-ux-review` — compact, 402×874, one hand: thumb reach, the 246pt panel's height budget, the island and the corners, which screen edge owns which axis.
    - `ipad-ux-review` — regular, 1376×1032 and 1032×1376: nobody reaches the middle, width is spare and height is not, pointer/keyboard/right-click as first-class, sheets at 1000pt.
    - `duo-ux-review` — inner 951×669 (regular width, **shortest screen we ship** — it passes every wide-layout gate and then has 669pt) and cover 466×678 behind an 84pt rail, plus the fold itself: what state survives opening and closing.
