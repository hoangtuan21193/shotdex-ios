...
/// corners and collapsed to a pill. It always reads the whole photo: not the part
/// currently on screen, and not the selected mask — a readout that changes meaning
/// when a mask is opened cannot be used to judge the frame's exposure.
struct EditorHistogramCard: View {
    ...
    let bounds: CGRect
    /// Shared with the collapsed pill in the top bar, so collapsing animates as
...
/// The collapsed histogram: a colour sparkline that lives in the top bar rather
/// than over the photo, so nothing covers the image while it is parked. No label
/// — at 34×16pt the coloured traces already say what it is.
    ...
    var body: some View {
        Button(action: onExpand) {
            EditorHistogramSparkline(histogram: histogram)
                ...
                .padding(.horizontal, 9)
                .frame(height: EditorLayoutMetrics.histogramCollapsedHeight)
                // Solid black, no border: parked right beside the Dynamic Island
                // it should read as an extension of it, not as a separate chip.
