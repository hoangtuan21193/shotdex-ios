...
- [x] C3 Widget — target `ShotDexWidget`: **On This Day** (small/medium/large/lock) và bốn widget vẽ trên ảnh người dùng chọn — **Clock**, **Calendar**, **Weather**, **Time, Date and Weather** (small/medium/large). Mọi thứ đọc từ App Group `group.com.hoangtuan.shotdex`; **widget không đụng thư viện ảnh, lịch, mạng hay vị trí**. Widget "Your Gear" đã bỏ (2026-09-20). Đã đặt lên màn hình chính và xác nhận bằng dữ liệu thật
...
- [x] C7 iPad — `TARGETED_DEVICE_FAMILY = "1,2"`, build/cài/chạy được trên iPad Pro 11", xoay ngang đã khai báo sẵn trong Info.plist; lưới đã tự co giãn theo regular width (`GridDensity.columns(forDensity:width:isRegularWidth:)`). Phím tắt trong viewer: `f` favorite, `i` info, `delete` xoá, `⌘S` share. **Đã soát tay trên iPad Pro 13" (2026-09-19)** — đi qua Library, Photo Detail, Editor (sidebar + Geometry), Collections, Smart Album Detail, Search, Settings, Places, Duplicates, Compare/Survey, Resize, Collage, Video Studio. Bốn lỗi bố cục đã sửa: (1) token album cắt chữ trong khi cạnh nó trống 800pt → rộng 240 ở regular width; (2) màn Search ghim capsule xuống đáy dù iPad để field ở toolbar → theo nhánh "field ở trên"; (3) form Resize kéo hết 1032pt → giới hạn 700pt căn giữa, và đổi tên hiển thị Compress → Resize cho khớp lối vào; (4) khung Collage kẹt ở trần 353 và timeline Video Studio nuốt 600pt trống → bỏ trần ở regular width / trần theo nội dung lane. Optics + Geometry cũng được trả lại sidebar editor vì giờ có Upright.
- [x] C8 Rà soát Dynamic Type — đo ở `accessibility-extra-large`: tab Collections **vỡ** (token cắt chữ, thumbnail đè tiêu đề), đã sửa bằng `@ScaledMetric` cho `AlbumTokenMetrics` + thẻ Memory + thẻ On This Day. Lưới ảnh, Settings, Statistics dùng font ngữ nghĩa nên đã tự scale. **2026-09-19**: nhãn tab bar tự dựng (pre-26) đã scale có trần và **bỏ nhãn ở cỡ accessibility, chỉ còn icon** — kiểm trên iPhone 16 Pro / iOS 18.6 ở `accessibility-medium` và cỡ mặc định; `OnboardingScreen` bọc `ScrollView` vì ở cỡ đó nó cắt cả bốn lời hứa thành "…". **Còn lại**: nhãn metadata trên tile (chrome tầng C, cố ý cố định)
