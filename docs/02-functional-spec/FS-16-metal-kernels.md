...
| AC-5 | build đủ 4 nhóm | chạy test nạp kernel | 40/40 nạp được; đổi tên một hàm kernel thì test đỏ | ⚠️ chưa có |
| AC-6 | metallib của kit nằm trong framework | test nạp một kernel kit | nạp từ bundle framework; bundle chính của app không có metallib của kit | ⚠️ chưa có |
| AC-7 | bản release, kernel không nạp được | render ảnh có mask | ảnh ra bằng ảnh vào, không crash | ⚠️ chưa có |
