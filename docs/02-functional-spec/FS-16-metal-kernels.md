...
| AC-2 | golden của một kernel đã port | render cùng ảnh vào bằng bản Metal | mọi pixel lệch ≤ 1/255 mỗi kênh; lặp cho cả 40 kernel | `KernelGoldenTests.matchesItsGolden` — Metal 6/40 (mask) |
...
| AC-5 | build đủ 4 nhóm | chạy test nạp kernel | 40/40 nạp được; đổi tên một hàm kernel thì test đỏ | `KernelLibraryTests.everyKernelLoads` (Metal: 6/40 sau nhóm 1) |
...
| AC-8 | point color với 8 điểm (tối đa của model) | render | điểm thứ 8 đổi màu đúng như golden 8 điểm | ⚠️ chưa có |
| AC-9 | ảnh thử 512×512, recipe bật mọi kernel nhóm 1–2 cùng lúc | render qua renderer đầy đủ | khớp golden toàn pipeline ≤ 1/255 | ⚠️ chưa có |
...
**Chưa chứng minh được:** AC-2 (34/40 kernel chưa port), AC-3, AC-5 (đủ 40 bằng Metal), AC-8, AC-9, AC-10.
