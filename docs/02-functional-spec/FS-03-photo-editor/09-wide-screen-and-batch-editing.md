...
|---|---|---|
| AC-1 | **Cho** một ảnh JPG và cửa sổ 1210×834 · **Khi** mở editor · **Thì** panel hiện đủ **5** tên section (Light · Color · Effects · Detail · Optics), tất cả **gập**, và header cuối cùng nằm **trên** mép trên của hàng `[Reset] [Save…]` — không phải cuộn để thấy tên nào | ⚠️ một nửa — `EditorPanelLayoutTests.thePanelListsFiveSections` (đúng năm section, đúng thứ tự) + `.everyWindowWithRoomGetsTheSidebar` (1210×834 qua cổng rộng); **chưa chụp** trạng thái gập mặc định |
| AC-2 | **Cho** cửa sổ Duo màn trong 951×669 · **Khi** mở editor · **Thì** histogram nằm **trong panel, ở trên cùng**, cao **56pt**, và hàng lệnh trên **không** có pill histogram | ⚠️ một nửa — `EditorPanelLayoutTests.theShortColumnShrinksChromeInsteadOfDroppingIt` (`sidebarHistogramHeight(forColumnHeight: 669) == 56`); **chưa chụp** trên Duo — máy đang gập, `simctl` không mở được màn trong |
| AC-3 | **Cho** Duo màn trong · **Khi** panel ở trạng thái mặc định · **Thì** đồ cố định (histogram 56 + mode header 40 + chân panel 74 + đường kẻ) ≤ **180pt**, vùng cuộn ≥ **430pt**, và **5 header gập (5×44 = 220pt) vừa trọn** trong vùng cuộn | ✅ `EditorPanelLayoutTests.theShortColumnShrinksChromeInsteadOfDroppingIt` — đồ cố định `≤ 180`pt và năm thẻ gập còn vừa vùng cuộn, tính thẳng từ hằng số bố cục |
| AC-4 | **Cho** Duo màn trong, panel mặc định · **Khi** mở section **Light** (8 slider) · **Thì** toàn bộ section cao **412pt** vừa trong vùng cuộn, và tên **Color** vẫn nhìn thấy | ✅ `EditorPanelLayoutTests.duoInnerIsAShortColumnAndTheIPadIsNot` — cột còn lại sau đồ cố định lớn hơn nhóm Light (header + 8 hàng), và **không** lớn hơn nếu còn histogram block + Look row như bản cũ |
...
| AC-6 | **Cho** Vignette = 0 · **Khi** nhìn section Effects · **Thì** Midpoint · Roundness · Feather · Highlights **mờ** theo `EditorTheme.rowDisabled` và **không nhận chạm** (kéo vào track không đổi giá trị) | ✅ `EditorPanelLayoutTests.dependentRowsWaitForTheirParent` — bốn hàng phụ của Vignette báo `isEnabled == false` khi cha bằng 0 |
| AC-7 | **Cho** Vignette = 0 · **Khi** kéo Vignette lên **+20** · **Thì** bốn slider phụ sáng lại và nhận chạm ngay, không cần đóng/mở section | ✅ cùng test — đặt `vignette = -0.2` thì `vignetteMidpoint` bật ngay, không qua bước đóng/mở section |
| AC-8 | **Cho** Grain = 0 · **Khi** nhìn Effects · **Thì** Size và Roughness mờ; đặt Grain = 25 thì sáng | ✅ cùng test — `grainSize`/`grainRoughness` tắt khi Grain = 0 và không bị Vignette đánh thức nhầm |
...
| AC-19 | **Cho** một hàng slider bất kỳ · **Khi** chạm cách vạch **20pt** theo chiều ngang · **Thì** cú chạm vẫn thuộc về slider đó (vùng bắt chạm ≥ 44pt), hình dạng vẫn là **vạch** chứ không phải núm tròn, và kéo từ mép vùng chạm đổi giá trị đúng bằng khoảng cách di chuyển (không nhảy) | ✅ `EditorPanelLayoutTests.onlyNearHorizontalPansBelongToASlider` (nghiêng 24° vẫn là slider, 40° thì không) + `.aSliderRowIsBigEnoughToHitAndQuickEnoughToNudge` (vùng chạm 44pt, ngưỡng 6pt cho ngón và 2pt cho bút) |
...
| AC-25 | **Cho** một ảnh chưa sửa gì · **Khi** nhìn hàng lệnh trên · **Thì** có pill **`Save`** nền accent chữ đen, **cao 32pt và rộng ≤ 96pt** (hôm nay: 240×50), và trong menu ⋯ dòng **`Reset All Adjustments`** đang **mờ** | ✅ `EditorPanelLayoutTests.primaryActionsAreSmall` — cao 32pt, rộng ≤ 96pt |
| AC-37 | **Cho** mode Crop & Geometry · **Khi** đo chân panel · **Thì** `Apply` là pill accent cao **32pt** rộng ≤ 96pt, `Cancel` là **chữ trần** không nền, `↺` là đĩa 32pt ở đầu kia, và cả chân panel cao **48pt** | ⚠️ một nửa — `EditorPanelLayoutTests.primaryActionsAreSmall` đo đúng pill 32/≤96; **chưa chụp** chân panel của Crop & Geometry |
...
| AC-31 | **Cho** iPad 11" **dọc** 834×1194 · **Khi** mở editor · **Thì** panel nằm bên cạnh ảnh (theo `sidebarEdge`) với đúng 5 thẻ như lúc ngang — **không** còn panel đáy 246pt và **không** còn bánh xe chip | ⚠️ một nửa — `EditorPanelLayoutTests.everyWindowWithRoomGetsTheSidebar` (834×1194 dọc vẫn qua cổng rộng, điện thoại và Duo ngoài thì không); **chưa chụp** hai hướng |
