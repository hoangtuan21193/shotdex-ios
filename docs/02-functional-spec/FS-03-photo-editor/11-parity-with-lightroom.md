...
| AC-5 | **Cho** ảnh có đúng một khuôn mặt · **Khi** tạo mask **Face Skin** · **Thì** mask phủ vùng da mặt và **không** phủ mắt, môi, chân mày | ✅ `FaceMaskTests` (hình học, gồm `faceSkinFollowsTheJawLine`); ảnh thật CC0: Vision của Mac → mask Skin/Eyes/Lips đúng chỗ (2026-09-23). ⚠️ Trên simulator landmark lệch — giới hạn Vision của sim, chụp trên máy thật còn nợ |
...
| AC-10 | **Cho** ảnh có chủ thể rõ · **Khi** tạo mask **Background** · **Thì** vùng chọn là phần bù của Subject, kiểm bằng tổng hai mask phủ kín khung | ✅ `MaskKindParityTests.backgroundIsSubjectInverted`, `addingBackgroundMaskInvertsSubject`, `invertedMaskIsTheComplement`; ảnh thật CC0 với Vision của Mac. ⚠️ Simulator không chạy được tách chủ thể |
| AC-11 | **Cho** ảnh **không có** bản đồ độ sâu · **Khi** mở danh sách tạo mask · **Thì** **Depth Range** mờ — cùng cổng `hasDepth` mà `depthBlur` dùng | ✅ `MaskKindParityTests.depthRangeIsGatedOnDepth`, `depthRangeSelectsTheBand`, `disparityIsStretchedToItsOwnRange`; ảnh HEIC có disparity tổng hợp trên iPad 26.5: dải gần được chọn, xa không |
