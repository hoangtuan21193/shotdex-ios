...
| AC-25 | s2 ghép xong, Size 100% ghi `W × H px` | kéo Size về 50% | dòng ước lượng ghi `round(W/2) × round(H/2)` (±1 px), MP còn ¼; ảnh lưu ra đúng kích thước đó | ⚠️ chưa có — `PanoramaRenderTests` + ảnh |
| AC-26 | dung lượng trống 500 MB, ảnh ra ước ~2 GB tạm + ảnh | mở panel | dòng ước lượng báo thiếu và thiếu bao nhiêu; Save mờ | ⚠️ chưa có — `PanoramaExportTests` + ảnh |
