...
**Trạng thái 2026-09-24** — 14 tiêu chí ✅, 15 ⚠️ một nửa, **0 ❌**. Mọi tiêu chí đều có code;
...
| AC-14 | 3 khung chỉ có trên iCloud, máy mất mạng | ghép | thông báo "3 photos couldn't be downloaded" + Retry; không lưu gì | ⚠️ một nửa — `PanoramaStitchServiceTests.framesThatWillNotLoadAreCountedNotDropped` đếm và nêu tên, nút **Retry** có trên màn; **chưa chụp** (cần sim tắt mạng với khung chỉ có trên iCloud) |
...
| AC-16 | s8 (méo thùng, không có hồ sơ ống kính) | ghép | hệ số méo ước lượng lệch ≤ 0,01; xoay lệch ≤ 0,2° | ✅ `PanoramaSolverTests.aLensWithNoProfileHasItsDistortionEstimated` (ước lượng lệch ≤ 0,01, xoay lệch ≤ 0,2°) + `.aStraightLensIsLeftAtZero`; hệ số đi thẳng vào kernel warp nên ảnh được nắn theo |
...
| AC-20 | đang lưu ở 30% | về màn hình chính 60 s rồi quay lại | lưu chạy tiếp, không bắt đầu lại; ra đúng 1 asset | ⚠️ một nửa — màn giữ sáng qua `ScreenAwakeCoordinator.beginHold` (không phụ thuộc setting), `beginBackgroundTask` giữ việc lúc vào nền, `PanoramaSaveRecord` + cảnh báo "Saving was interrupted" ở Library cho mở lại đúng bộ khung; **chưa kiểm trên máy thật** ở iOS 26 và 18 |
...
| AC-22 | chọn 120 khung | ⋯ → Combine Photos ▸ Panorama | ước lượng thời gian hiện trước khi ghép; Cancel đóng màn, không làm gì | ✅ ảnh iPhone 17 Pro: chọn 63 ảnh → "This will take a while / About 1 minute, 15 seconds for 63 photos." với Cancel · Continue; dưới 50 khung ghép thẳng |
