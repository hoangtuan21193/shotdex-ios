...
- Vào chế độ chọn → cụm control Liquid Glass (`SelectionBottomBar`) **thế chỗ tab bar ngay tại vị trí đó** và **crossfade tại chỗ** với tab bar (không trượt): bar KHÔNG còn là overlay per-screen — screen đang chọn publish `SelectionBarConfig` (count/thumbnail ids/onCompare/onDelete/onDeselect/isDeleting) lên `AppNavigation.selectionBar` qua một `.onChange` trên snapshot Equatable của selection; **root `HomeTabScaffold` host bar** trong đúng slot đáy của tab bar. Pre-26: trong `ZStack` đáy, đổi giữa `LiquidGlassTabBar` ↔ `SelectionBottomBar` theo `selectionBar != nil`, cùng `.padding(.bottom, 8)`, `.transition(.opacity)` + `.id` riêng, một `.animation(.snappy)` → crossfade một transaction. iOS 26: `.overlay(alignment:.bottom)` trên `TabView` (tab bar native tự ẩn bằng `.toolbar(.hidden, for: .tabBar)` cục bộ mỗi screen) — best-effort tại chỗ vì hệ tự animate ẩn bar theo curve riêng. Scaffold đo chiều cao bar (`measureHeight`) và mirror một chiều vào `navigation.selectionBarHeight`; grid mỗi screen nội `bottomInset` (On This Day: spacer đáy) bằng giá trị đó khi đang chọn nên hàng cuối cuộn qua khỏi bar → chọn được. `hidesTabBar` giữ lại như cờ legacy (scaffold pre-26 không còn đọc nó cho việc đổi bar). **Bố cục**: **Share** chuyển LÊN toolbar top-leading khi đang chọn (thế chỗ nút Settings gear ở Library; các screen album đứng cạnh nút back hệ thống) — là toolbar Button native nên tự có Liquid Glass; đang gom ảnh thì thay glyph bằng `ProgressView`. **Bar mô phỏng hình dạng tab bar** (`LiquidGlassTabBar` = `HStack { pill; nút tròn }`) nên trông như tab bar biến hình: 3 phần (trái→phải) **chip Compare** (pill glass rời) + **pill giữa** (`GlassPanel` **pill** bo tròn hết, corner = `selectionPanelCorner` 26 = nửa chiều cao) chứa thumbnail + nút **Delete** tròn (`GlassIconButton` tint đỏ) ở đúng slot nút search. Cả cụm **căn giữa đáy** (không giãn sát 2 mép) — giống tab bar native là cụm nổi ở giữa (pill + nút search sát nhau), nên lúc select ẩn tab bar native rồi cụm này đè đúng chỗ pill cũ. **Slot ảnh tròn đồng tâm với pill**: corner `selectionSlotCorner` = 26 − 8 (padding dọc panel) = 18 = nửa cạnh slot 36 → hình tròn lồng trong capsule. **Compare** là một `GlassPanel` **bo tròn hết** (cornerRadius lớn = capsule, giống chip indexing) riêng bên TRÁI panel, chữ `lineLimit(1)`/`fixedSize`, **luôn hiện** (khi feature có, tức `onCompare != nil`) ghim sát mép trái với **margin cân bằng** (khoảng cách tới viền màn hình = padding bar 16 = spacing 16 tới panel) — **không bay ra/thu vào nữa**. Chỉ **enable trong khoảng 2–4** ảnh (`compareEnabled`); ngoài khoảng đó (0–1 hoặc >4) `.disabled` + chữ `.tertiary` (xám mờ) để yêu cầu "2–4 ảnh" dễ hiểu chứ không biến mất. Khi chọn, Library ẩn nút Filter/Sort, On This Day ẩn nút calendar. **Pill giữa** căn giữa (Compare không có ở On This Day thì vẫn giữa nhờ `Spacer`). ≤4 ảnh → 4 slot cố định; slot có ảnh hiện thumbnail local **hình tròn** (kèm badge `xmark.circle.fill` nhỏ **tràn ra ngoài góc trên phải**, `offset(6,-6)`, chỉ che một sliver ảnh, làm gợi ý bỏ chọn), slot trống là **ô tròn viền dash** cùng shape thumbnail (corner 18 chung `selectionSlotCorner`) có icon `photo`; **>4 ảnh → panel GIỮ nguyên cửa sổ rộng đúng 4 slot** (không tràn full-width; Compare ẩn, panel vẫn căn giữa) và biến thành `ScrollView` ngang TẤT CẢ thumbnail đã chọn theo thứ tự chọn (không stack đè) — vì cửa sổ chỉ 4 slot nên ảnh thứ 5 đã tràn, `ScrollView` dùng **`.defaultScrollAnchor(.trailing)`** (iOS 17+) nên tự ghim mép phải mỗi khi content đổi kích thước → hiện ảnh mới nhất (phải cùng) **ngay mỗi lần thêm ảnh** (không lệ thuộc `scrollTo` trễ một frame), vừa để lộ rằng cuộn được. **Chạm thumbnail → bỏ chọn ảnh đó** (`onDeselect`; slot trống không phản hồi). **Dòng text đếm** nằm **dưới cả cụm** (không trong pill): `canCompare` → 0 ảnh "Select up to 4 to compare", 1–4 ảnh "N selected · up to 4 to compare", >4 ảnh chỉ "N selected"; không compare được (On This Day) → "Select Photos" / "N selected". **Share** (`PhotoShareSheet`, dùng chung với Photo Detail): gom item mọi ảnh chọn — ảnh → data, video → URL, **cho tải iCloud-only** (`isNetworkAccessAllowed = true`), rồi mở một `UIActivityViewController`. **Delete** (destructive, `deleteAssets`: hệ thống hiện dialog xác nhận, prune DB + state local, user hủy → giữ selection). On This Day là delete-only nên truyền `onCompare: nil` (Compare luôn ẩn, vẫn có Share). **Glass chrome**: `GlassIconButton` + `GlassPanel` dùng `.glassEffect` native trên iOS 26 (không stroke/shadow thủ công), fallback `.ultraThinMaterial` + viền pre-26 — nên Delete/capsule trông đúng Liquid Glass giống nút search/back chứ không mờ đục. Sau cụm control, **scaffold** đặt **`BottomScrim`** neo **đáy màn hình thật** (full-bleed, `ignoresSafeArea`, không chặn tap) khi `selectionBar != nil` — chỉ gradient đen nhạt mờ dần từ đáy lên (~0.32, không blur) cho tối nhẹ để text đếm + pill dễ đọc trên ảnh sáng
