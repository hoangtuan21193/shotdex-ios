...
- Thumbnail size theo cell width tính từ bounds collection view + số cột ở **native display scale 2x/3x**; khi cell to lên (pinch về ít cột) cell re-request bản resolution cao hơn (ngưỡng 1.4×), giữ ảnh cũ tới khi bản nét về. (On This Day vẫn dùng SwiftUI `PhotoGridTile` + `.measureWidth(into:)` — grid nhỏ 3 cột cố định)
...
- **Swipe detail local-first rồi nâng lên screen-size**: mọi page được dựng trước fire thumbnail opportunistic/aspect-fit và `requestBestLocalImage` local-only. Riêng page visible còn chạy song song một đường local xác định: `requestImageDataAndOrientation` với network tắt; nếu current-version bytes thật sự nằm trên máy, ImageIO downsample trực tiếp xuống đúng aspect-fit pixel màn hình (`localOriginal`), không decode full image vào view và không thể nhầm grid rendition. Nếu original offload thì đường này trả nil, không download. Grid `UICollectionView` preheat local-only rendition cỡ màn hình cho các cell đang visible (`willDisplay` start / `didEndDisplaying` stop), nên resize thường hoàn tất trước lúc tap. Detail **không render callback thiếu pixel**: thumbnail/local/final phải đạt ít nhất 90% target aspect-fit; nếu chưa có ảnh đạt chuẩn thì giữ nền đen + progress thay vì phóng grid thumbnail thành ảnh mờ. `requestDetailImage` opportunistic/exact chạy ngay khi visible; trang đầu/reseat set active trước mount. Derivative thiếu pixel retry một lần ở target 2×. Neighbour page local-only, không tranh network. Quality state tăng đơn điệu `local < localOriginal < displayFinal < original`; cache tách theo tier.
...
- Chọn nhiều: long-press (hoặc nút toolbar) vào chế độ chọn nhiều (không giới hạn số lượng, badge checkmark, hỗ trợ swipe-to-select xuyên section — range tính trên mảng ảnh phẳng), bottom bar **giống các album khác** (Compare + thumbnails + Delete; Share ở toolbar). Selection lưu bằng `Set<String>` (union/subtract cho swipe), nên preview thumbnail và panes Compare theo **thứ tự hiển thị** của grid (`orderedSelection` = lọc `controller.photos` theo id đã chọn) chứ không phải thứ tự set. **Compare** (`onCompare` mở `CompareScreen` qua `fullScreenCover`, `onDismiss: stopSelecting`) enable trong `2...CompareScreen.maxPhotoCount`. **Delete** → `PhotoLibraryService.deleteAssets` (`PHAssetChangeRequest.deleteAssets`, hệ thống tự hiện dialog xác nhận, ảnh vào Recently Deleted). Sau khi xoá thành công, xoá luôn row tương ứng trong DB (`MetadataDAO.deleteAssets`) để Library grid không stale; user hủy dialog → giữ nguyên selection.
