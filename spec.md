...
- **Swipe detail local-first rồi nâng lên screen-size**: mọi page dựng trước fire thumbnail opportunistic/aspect-fit và `requestBestLocalImage` local-only. Page visible còn thử current-version bytes local rồi ImageIO downsample; original offload thì đường này nil, không download. Grid preheat tối đa 18 cell visible: PhotoKit caching bắt đầu ở `willDisplay`, còn decoded `requestBestLocalImage` trì hoãn 180ms (cell lướt nhanh sẽ rời trước khi decode); bắt đầu drag hủy decoded warm-up, scroll dừng mới tiếp tục. Nhờ vậy proxy dùng chung thường sẵn trước tap mà không giành decode trong lúc scroll. Detail phân hai ngưỡng theo pixel thực: grid thumbnail dưới **65%** target không được phóng full-screen; local display proxy ≥65% (thường rendition ~1024 px trên màn hình ~1290 px) hiển thị ngay và được final ≥90% thay âm thầm. Spinner có grace **350 ms**, nên cache hit/proxy local không nháy vòng xoay; chỉ hiện nếu thật sự chưa có proxy dùng được. `requestDetailImage` exact chạy ngay khi visible; derivative thiếu pixel retry target 2×. Neighbour page local-only, không tranh network. Quality state tăng đơn điệu và cache tách theo tier.
