...
- **Chữ trong sidebar scale theo Dynamic Type** (`EditorTheme.sidebarTitle/sidebarGroupLabel/sidebarActionLabel`, header section dùng `@ScaledMetric`). Slab 246pt của điện thoại vẫn giữ size cố định theo §7.2 — nó không có chỗ để giãn; sidebar thì đã bỏ hết chiều cao cố định nên giãn được.
