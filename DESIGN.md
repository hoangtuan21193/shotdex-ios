...
**Nút tròn/pill trong tầng D** (Compare, Video Studio, editor) dùng `.editorGlass(_ shape:)` — kính **tối**, không dùng `.glassBackground` (kính sáng tầng C, chữ trắng khó đọc trên nền tối). Màu glyph theo một quy tắc: **inactive** = trắng (`.white` / `.white.opacity(0.9)`), **active** = nền accent + glyph **đen**, **disabled** = `.white.opacity(0.28)`. **Trong photo editor, active = nền trắng + glyph đen** (FS-03.12): accent chỉ còn trên Save. Không trộn glyph accent với glyph trắng cho cùng một vai trò. Trạng thái **đánh dấu phá hủy** (card Compare mở từ Duplicates: viền đỏ 3pt quanh card, nút chữ đổi `Delete` → `Keep`) = đĩa `.red` + glyph trắng — bản phá hủy của "active", không dùng accent cho nó.
...
Ngoại lệ có phạm vi của §10.1c. Một phần tử **chỉ** được to lên theo cửa sổ khi **đủ cả ba**:

1. nó là **thứ để nhận ra** một bộ sưu tập / một khoảnh khắc — tức một bức ảnh bìa, không phải chrome;
...
- **Hero On This Day chiếm hết bề ngang và cao theo size class**: **180 (compact) → 260 (regular)** (`CollectionsHeroMetrics.height`), `@ScaledMetric` đặt ở màn chứ không ở thẻ. Đây là **lập luận riêng**, không mượn §10.1d (hero là banner đơn độc, không thoả điều kiện 2 và 3): nó đã full-bleed nên bề rộng tự lớn theo màn, và nếu **chỉ** bề rộng lớn thì ở 1032pt nó thành một **dải letterbox 150pt** — tỉ lệ ~7:1, không còn đọc ra là một bức ảnh. Chiều cao tăng là để giữ tỉ lệ ở mức xem được, và dừng ở 260 chứ không theo tỉ lệ thật, vì hero không được ăn hết màn đầu tiên. Section "Recents" riêng bị bỏ.
...
- **`List(selection:)` của sidebar phải là con trực tiếp của cột sidebar** (2026-09-22, đo trên iPad): bọc nó trong một `if`/`@ViewBuilder` — ví dụ để đổi sang danh sách kết quả tìm kiếm — thì **chạm vào một mục không làm gì cả**, vì SwiftUI chỉ trao hành vi chọn-bằng-một-chạm cho chính list của sidebar. Muốn đổi nội dung khi tìm kiếm thì đổi **hàng bên trong** list, đừng thay cả list.
...
**Panel điện thoại của photo editor** (FS-03.12, 2026-09-24) — một ngôn ngữ cho cả panel:
...
- **Accent chỉ trên Save** trong toàn photo editor — bánh xe (chip giữa trắng + vạch 16×2), nút đang giữ trên băng (đĩa trắng), chip, slider, công tắc đều không dùng accent. Rail, header và Apply của sidebar là ngoại lệ còn lại (bố cục sidebar không đổi).
- Không dấu "đã chỉnh" nhìn thấy trên panel điện thoại; VoiceOver vẫn đọc "Edited".
- Cờ `\.editorUsesPanelStyle` bật kiểu này **chỉ** trong panel và sidebar của photo editor; Collage, Video Studio và các tool tầng D khác giữ kiểu chip/slider cũ.
...
- **Điều kiện vào layout này**: rộng ≥700, cao ≥600 **và rộng > cao** (`EditorLayoutMetrics.usesSidebar`). Điều kiện hướng có từ 2026-09-21: sidebar trả bằng **bề rộng**, thứ chỉ cửa sổ ngang mới dư — iPad dọc qua hai con số kia rồi để ảnh 3:2 còn **34% chiều cao canvas**, so với 70% khi ngang. Dọc giữ slab điện thoại; nó tiêu chiều cao, thứ ảnh letterbox không dùng.
- **Cột ngắn (<800pt) gập hai hàng**: khối histogram và hàng Look, trả lại 165pt, pill histogram trên band bật lại bù. Màn trong Duo (951×669) là ca này: không gập thì riêng nhóm Light (412pt) đã cao hơn cả viewport (300pt).
- Bố cục (2026-09-21): `[top bar toàn chiều ngang]` trên, dưới là `[rail 48pt][panel 280–420][canvas + filmstrip]`. Rail và panel **cùng một cạnh**, trái hay phải do người dùng chọn (menu ⋯ → Tools Panel, mặc định phải); panel **kéo đổi rộng 280–420pt** và **thu gọn được**, rail thì không. Cả ba lưu `@AppStorage`.
- **Top bar luôn hiện, và soi gương theo cạnh của panel**: cụm lệnh luôn nằm trên phía có panel, Back đứng **một mình** ở cạnh đối diện — để panel bên trái mà không soi gương thì undo cách slider đang cầm 800pt trên màn 13". Band chỉ được có **một** phần tử co giãn: hai cái thì `HStack` chia đôi slack và cụm lệnh trôi ra giữa. Nguyên bản: Back đứng **một mình** ở mép trái (nó là nút duy nhất *rời khỏi* phiên sửa — xếp chung cụm với các nút đổi ảnh là cách một phiên bị vứt đi vì chạm nhầm), rồi undo · redo · before-after · fit-fill · ⋯ · Save ở mép phải. Cao `52 + safeArea.top`, hàng nút bắt đầu ở `safeArea.top + 4` vì editor đã nhận hết safe area cho ảnh.
- **Rail dọc icon-only 48pt** ở cạnh ngoài, **không bao giờ thu gọn**: Edit · Presets · Crop · Mask · Markup, rồi History + nút ẩn/hiện panel ở đáy. Đây là **radio**, chạm lại cái đang bật để về Edit. Lý do nó là cột chứ không phải dải ngang trong panel: năm mode loại trừ nhau (chỉ một cái giữ được crop frame / mask overlay) nên tam giác xổ là nói dối về chúng; và một cột tiêu **bề rộng** — thứ canvas ngang đang dư — thay vì 44pt **chiều cao** panel, thứ danh sách tham số luôn thiếu. Panel thu gọn thì rail vẫn là đường quay lại công cụ.
- **Panel từ trên xuống**: **histogram luôn hiện** (đọc liên tục lúc kéo slider, không giấu sau một cú chạm; cố ý **không** thả nổi trên ảnh như Lightroom — biểu đồ mà di chuyển là biểu đồ phải đi tìm lại) → hàng tiêu đề mode (`EDIT`/`CROP`/… + nút **Auto**) → hàng **Look** (tên film look + `Browse`, chỉ ở mode Edit) → nội dung → chân panel `[Reset ↺][Save…]`.
- **Mode Edit là chồng section xổ được**, mở bao nhiêu cùng lúc cũng được, xếp **theo thứ tự pipeline**: Light → Curve → Color → Grade → Effects → Detail → Optics → Geometry. Chevron nằm **đầu** hàng (đọc tam giác trước tên là một lần liếc, không phải nhảy ra lề rồi quay lại) và **không có icon group** — rail đã mang icon. `Mix` và `Point` là **segment bên trong Color** (Lightroom lồng HSL y hệt). Bốn mode còn lại **thay** cả chồng section, không chèn lên trên nó: rail đã nói đang ở mode nào, tám header bên dưới chỉ là danh sách những thứ tool này *không phải*.
- **Slider trong panel xếp dọc** (`\.editorSliderStacked`, hàng 46pt): tên + số ở dòng trên, **track full-width** ở dòng dưới. Xếp ngang trong panel 320pt thì 88pt là nhãn, 44pt là số, track còn một phần ba — mà track là thứ duy nhất ngón tay nhắm vào. Slab 264pt của điện thoại giữ hàng ngang 40pt: ở đó chiều cao mới là thứ khan hiếm.
- **Color Mix chọn dải trước** ở tier này: hàng 8 swatch tròn 22pt + chốt "All", rồi Hue/Saturation/Luminance của dải đang chọn. Swatch là hình tròn màu chứ không phải chip chữ vì thứ đang chọn **là** một màu — chữ phải đọc, màu thì nhận ra. 24 hàng cao hơn panel; chọn dải trước còn 3 hàng nhìn thấy cùng lúc.
...
- **Top bar thay cho hàng tiêu đề + hàng lệnh trong panel** (đổi 2026-09-21). Trước đó band chỉ hiện lúc panel thu gọn, với lý do "hai dải chrome là ăn 56pt của ảnh"; thực tế panel đang trả cho đúng nhóm lệnh đó hai lần — một hàng tiêu đề 44pt và một hàng lệnh 44pt mà nó **không cuộn được**. Bar lấy 52pt chiều cao của ảnh và trả lại 88pt cho danh sách tham số. Pill histogram trong bar **tắt** khi panel mở: cùng một biểu đồ vẽ hai lần cách nhau 40pt.
...
- **Apple Pencil** (2026-09-19): `PKCanvasView.drawingPolicy = .default` (không phải `.anyInput`) và brush mask ưu tiên `touch.type == .pencil` — Pencil vẽ, ngón tay pan, lòng bàn tay bị bỏ. Trước đó bàn tay chạm trước thành nét vẽ, Pencil tới sau bị tính là ngón thứ hai. **Ngoại lệ: panel điện thoại** (FS-03.12) ẩn `PKToolPicker`, mà `.default` không có công tắc "vẽ bằng ngón tay" của picker thì từ chối ngón tay — ở đó dùng `.anyInput`.
...
- **Chữ trong sidebar scale theo Dynamic Type** (`EditorTheme.sidebarTitle/sidebarGroupLabel/sidebarActionLabel`, header section dùng `@ScaledMetric`). Slab 264pt của điện thoại vẫn giữ size cố định theo §7.2 — nó không có chỗ để giãn; sidebar thì đã bỏ hết chiều cao cố định nên giãn được.
- **Panel trong sidebar không tự xưng tên** (`\.editorPanelShowsTitle == false`): hàng tiêu đề mode cách đó 40pt đã ghi "MASK" rồi, panel ghi "Masks" lần nữa bằng font to hơn là cùng một tên hai lần.
- **Filmstrip bản thấp** ở tier này (thumbnail 56 / dải 72, thay 96 / 124 của điện thoại): sidebar đã tiêu bề rộng, dải không nên tiêu thêm một phần mười chiều cao — và 56pt vẫn đủ để chọn giữa hai kiểu bấm, việc duy nhất dải đó làm.
- **Không đặt `.contextMenu` lên stage ở compact width.** `UIContextMenuInteraction` cướp đúng cú nhấn mà `holdBeforeGesture` đã nhận ở 0.3s, nên `onEnded` không chạy và `showsOriginal` **kẹt bật cả phiên** — đo được: một cú giữ 1.2s để lại nút before/after màu accent sau hai màn, và mọi slider sau đó không thấy đổi gì. Menu chỉ dựng khi `horizontalSizeClass == .regular`; ngoài ra mọi lần ghi recipe đều clear `showsOriginal` phòng thủ.
- **Mọi nút trong band phải có `.hoverEffect`.** `.buttonStyle(.plain)` không tự có hiệu ứng con trỏ; panel sáng lên còn band cách đó 40pt thì không, và chính sự lệch đó đọc ra như band hỏng.
- **Track có màu** cho Temp, Tint, **Saturation và Vibrance** (`EditorTheme.troughGradient`): xám ở trái chạy sang đủ màu ở phải. Track màu thì bỏ phần fill (trắng 50% trong photo editor, accent ở các tool khác) — màu **là** nghĩa của slider đó. **Ngoại lệ có lý do: gradient nào không trung tính tại anchor thì giữ một vệt trắng 2pt từ anchor tới con trỏ** (`troughIsNeutralAtAnchor` → `showsTrailOverGradient`). Temp/Tint xám ở giữa, đúng chỗ anchor, nên bỏ fill vẫn đọc được "đang ở 0"; ramp của Sat/Vibrance sáng nhất ở giữa, nên bỏ fill thì một hàng ở 0 và một hàng ở +40 trông y hệt và con trỏ 4pt là toàn bộ readout.
- **Toast và mọi thứ nổi ở đáy phải đậu *trên* panel, không đè nó.** Toast undo canh đáy 24pt thì nằm y 806–850 trên máy 402×874, còn bánh xe nhóm ở 795–849 — chồng hoàn toàn, và toast có nút riêng nên nuốt tap chứ không cho xuyên. Vùng letterbox trên panel là chỗ trống không hit-test.
- **Fade hai đầu một dải cuộn ngang phải rộng ít nhất bằng một ô.** 26pt hẹp hơn phần chip bị cắt nên vết cắt rơi vào chữ và đọc ra như chữ hỏng ("ight", "Point Co") thay vì "còn nữa, kéo đi".
- **Thứ gì phải nhìn ảnh mới chọn được thì không đặt trong sheet.** History là ca mẫu: chọn một bước là nhìn tấm ảnh nó tạo ra. Ở layout rộng nó là một cột trong panel (chặng trên rail, header `HISTORY` + `Done`); điện thoại mới dùng sheet, vì ở đó không có panel. Thêm một lý do cứng: cửa sổ **regular-width nhưng compact-height** (màn trong Duo, 951×669) làm UIKit **bỏ qua `presentationDetents`** — mọi detent đều thành full-screen — nên trên đúng thiết bị cần thấy ảnh nhất thì sheet che sạch. Ngược lại, thứ *chọn xong rồi thôi* (định dạng lưu, kiểu mask) cứ để sheet.
- **Hỏi safe area, đừng hỏi idiom.** `hasTopCutout` phải là `safeArea.top > 24`: iPhone Duo báo `.phone` ở cả hai màn, nên band trên cover chừa 125pt (một phần ba của 382) cho một Dynamic Island không nằm ở đó.
...
- **Settings là màn hình đầy đủ** (`fullScreenCover`, 2026-09-21), không còn sheet: nó đã dài hơn một sheet chịu được, và trình sửa widget bên trong nó có một preview phải kéo/pinch — sheet sẽ nuốt mất cử chỉ đó. Một bề mặt vừa là nội dung vừa là control (preview widget) thì **ghim trên cùng, không cuộn**, phần chỉnh nằm dưới.
...
- **Danh sách điểm-đến là hàng một dòng, và màn rộng thì chia cột** (2026-09-20): với một nhóm **điểm đến chọn từ danh sách biết trước** (Media Types, Utilities, Creations) thì mỗi mục là **một thẻ full-width cao đúng một dòng text** — glyph nhỏ ở đầu, tên, số, chevron — chứ không phải token có cover cuộn ngang: token tiêu một thẻ 60×190 và một ô ảnh cho mỗi mục, giấu một nửa sang bên phải, mà tên dài vẫn cắt. Nhưng **một hàng trải hết 1032pt là layout điện thoại bị phóng to** (§10.1c): số đứng cách tên nó 800pt. Nên hàng nằm trong `LazyVGrid` `.adaptive(minimum:)` với ngưỡng bằng bề rộng nội dung của một chiếc điện thoại (**320**): điện thoại ra đúng 1 cột, iPad 13" ra 3 cột. Màn rộng = **nhiều mục thấy cùng lúc**, không phải mục to hơn.
...
- **Trắng mờ của tầng D có tên, không viết literal** (2026-09-20): ngoài `hairline` 0.09 / `panelDivider` 0.07 / `panelTopHairline` 0.10 đã có, thêm ba tên cho timeline — **`trackBorder` 0.12** (viền clip chưa chọn, đường nền dưới ruler), **`trackChip` 0.08** (nền chip trung tính: pill timecode, pill inspector chưa chọn), **`emptyLane` 0.04** (rail của lane chưa có gì — khẽ hơn divider vì nó là *một chỗ*, không phải một cạnh). Ba giá trị này trước đó bị gõ lại rải rác 9 chỗ. **Bổ sung 2026-09-20** cùng bố cục bàn dựng: **`overviewClip` 0.22** (một clip trong dải project-overview — sáng hơn `trackChip` vì dải chỉ cao 22pt và đọc lướt, fill hợp cho ô clip 88pt thì biến mất ở đây), **`overviewWindow` 0.50** (khung đánh dấu cửa sổ mà timeline đang phóng to), **`meterPeak` 0.75** (tick peak-hold của level meter khi còn dưới ngưỡng đỏ). `DESIGN.md` không có thang opacity dạng nấc: thiếu thì **đặt tên trong `EditorTheme`**, không chọn bừa một số trong file feature.
...
- **Ô có kích thước cứng mà bên trong là chữ thật thì kích thước đó phải `@ScaledMetric`** (2026-09-19). Thẻ một dòng của Media Types/Utilities (`CollectionListRowMetrics`: 52 cao × 190/240 rộng) và cover tile (`AlbumTileMetrics`), thẻ Memory (260×150) và thẻ On This Day (cao 150) đều scale theo `.subheadline`/`.headline`. Đo ở `accessibility-extra-large` trước khi sửa: "Recently Viewed" ra "Rece…", thumbnail đè lên tiêu đề, câu dưới On This Day cụt. Hàng `LazyHGrid` chứa token phải scale **cùng một con số**, nếu không hàng sẽ cắt token bên trong.
